...
class CustomUser(AbstractUser):
    ...
    email = models.EmailField(
        verbose_name="email address", unique=True, max_length=60)
...
class OauthInfo(CreationBaseModel):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="uauth")
    secret = models.CharField(
        verbose_name="Oauth secret", max_length=64, unique=True, blank=True, null=True)
    uri = models.CharField(verbose_name="Oauth uri",
                           max_length=2500, unique=True, blank=True, null=True)
